const indexOf = (buf: Buffer, val: Buffer, byteOffset: number): number =>
{
    let match = -1;

    for (let i = 0; byteOffset + i < buf.length; i++)
    {
...
const bufferIndexOf = (buf: Buffer, val: Uint8Array| Buffer | string | number, byteOffset = 0): number =>
{
